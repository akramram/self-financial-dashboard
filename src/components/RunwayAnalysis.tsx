  ...
  return (
    <div className="relative flex flex-col items-center justify-center">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">bulan</span>
      </div>
    </div>
  );
}
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs">
        {sorted.map((b, i) => (
          <div key={i} className="flex items-center gap-1.5">
            ...
            <span className="font-medium text-slate-700 dark:text-slate-200">{Math.round(b.liquidityPct * 100)}%</span>
          </div>
        ))}
      </div>
    </div>
      ...
      <Card className="border-slate-200 dark:border-slate-700 shadow-none">
        <CardContent className="p-5">
          <div className="animate-pulse space-y-3">
            ...
            <div className="h-40 w-40 mx-auto bg-slate-200 dark:bg-slate-700 rounded-full" />
          </div>
        </CardContent>
      </Card>
    );
    ...
    <Card className={`border ${config.borderColor} shadow-none`}>
      ...
      <CardContent className="space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-sm">
              ...
              <div className="flex-1">
                <div className="text-slate-500 dark:text-slate-400 text-xs">Aset Likuid</div>
                <div className="font-semibold text-slate-800 dark:text-slate-100">{formatIdr(data.liquid_assets)}</div>
              </div>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <Wallet className="w-4 h-4 text-purple-500 flex-shrink-0" />
              <div className="flex-1">
                <div className="text-slate-500 dark:text-slate-400 text-xs">Total Aset</div>
                <div className="font-semibold text-slate-800 dark:text-slate-100">{formatIdr(data.total_assets)}</div>
              </div>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <CalendarClock className="w-4 h-4 text-amber-500 flex-shrink-0" />
              <div className="flex-1">
                <div className="text-slate-500 dark:text-slate-400 text-xs">Pengeluaran/Bulan (rata-rata 3 bln)</div>
                <div className="font-semibold text-slate-800 dark:text-slate-100">{formatIdr(data.monthly_expense)}</div>
              </div>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <ShieldCheck className="w-4 h-4 text-emerald-500 flex-shrink-0" />
              <div className="flex-1">
                <div className="text-slate-500 dark:text-slate-400 text-xs">Cakupan Biaya Tetap</div>
                <div className="font-semibold text-slate-800 dark:text-slate-100">{data.fixed_coverage_months.toFixed(1)} bulan</div>
              </div>
            </div>
          </div>
        </div>
        ...
        {data.asset_breakdown.length > 0 && (
          <div>
            <div className="text-xs text-slate-500 dark:text-slate-400 mb-2">Likuiditas Aset</div>
        ...
        {!compact && trendSparkData.length >= 2 && (
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs text-slate-500 dark:text-slate-400">Tren Runway</div>
              <div className="flex items-center gap-1">
                <TrendingUp className="w-3 h-3 text-slate-400" />
                <span className="text-xs text-slate-400">{data.history.length} bulan terakhir</span>
              </div>
            </div>
        ...
        {!compact && data.tips.length > 0 && (
          <div className={`rounded-lg ${config.bgColor} p-3 space-y-1.5`}>
            ...
            {data.tips.map((tip, i) => (
              <p key={i} className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">{tip}</p>
            ))}
          </div>
        )}
