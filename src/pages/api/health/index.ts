...
export const GET: APIRoute = async ({ url }) => {
  ...
  factors.push({
    ...
    score: budgetScore,
    maxScore: 20,
    label: catsWithBudgets.length > 0
      ? `${overspentCats.length}/${catsWithBudgets.length} over budget`
      : 'No budgets set',
    detail: overspentCats.length > 0
      ? `Over budget: ${overspentCats.join(', ')}`
      : catsWithBudgets.length > 0
        ? 'All categories within budget limits'
        : 'Set category budgets for better tracking',
    icon: 'target',
  });
  ...
  // ── Factor 4: Spending Control (0-20) ─────────────────────────────
  ...
  if (income > 0) {
    ...
    else if (spendRatio <= 1.0) spendingScore = 4;
  ...
  factors.push({
    ...
    detail: income > 0
      ? totalSpent <= income * 0.7
        ? 'Spending well under control'
        : totalSpent <= income
          ? 'Spending within income limits'
          : 'Spending exceeds income!'
      : 'Set income to track spending ratio',
    icon: 'wallet',
  });
  ...
  if (overall >= 90) { grade = 'A+'; gradeColor = 'emerald'; }
  else if (overall >= 80) { grade = 'A'; gradeColor = 'emerald'; }
  else if (overall >= 70) { grade = 'B'; gradeColor = 'green'; }
  else if (overall >= 60) { grade = 'C+'; gradeColor = 'yellow'; }
  else if (overall >= 50) { grade = 'C'; gradeColor = 'amber'; }
  else if (overall >= 40) { grade = 'D'; gradeColor = 'orange'; }
  ...
  if (monthIdx > 0) {
    ...
    if (prevIncome > 0) {
      ...
      else if (prevRatio <= 1.0) prevSpendScore = 4;
